...
DROPOUT_VEC_TRAIN = [1.0] * 8 + [0.8] * 2 + [0.5] * 2 + [0.5] * 1 + [0.5, 1.] * 5
# + ([0.6, 1.0] * 2 + [0.5, 1.0] * 1 + [0.5, 1.0] * 1 + [0.43, 1.0] * 2 + [0.5, 1.0] * 1)
# origin val = [1.0] * 8 + [0.7] * 2 + [0.5] * 2 + [0.5] * 1 + [0.5, 1.] * 5
...
def load_imitation_learning_network(input_image, input_data, mode):
    ...
    with tf.name_scope('Network'):  # for a nicer Tensorboard graph, use: `with tf.variable_scope('Network'):`
        ...
        """Start BRANCHING"""
        # TODO branch_idx 에서 빼주는 값 적절하게 변경(1 or 3)
        # TODO preprocessor 에서 hlc masking 값 변경
        branch_config = [
