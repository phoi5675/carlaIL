...
def model(features, labels, mode, loss_weights):
    ...
    pred_all_branches = tf.stack(branches[:ilc.SPEED_BRANCH_INDEX], axis=1)  # shape: (?, 4, 3)
    ...
    if mode in [tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL]:
        ...
        with tf.name_scope('predictions'):
            ...
            # high_level_cmds are [2, 3, 4, 5]; convert to 0-indexing -> [1, 2, 3, 4, 5, 6] for 0.9.X
            branch_idx = tf.cast(labels[ilc.TGT_HIGH_LVL_CMD], tf.int32) - 1  # shape: (?,)
            ...
            print("\n")
            '''
            print(sess.run(tf.range(batch_size)))
            print(sess.run(branch_idx))
            print(sess.run(tf.cast(labels[ilc.TGT_HIGH_LVL_CMD], tf.int32)))
            print(sess.run([tf.range(batch_size), branch_idx]))
            
            '''
            print(sess.run(labels[ilc.TGT_HIGH_LVL_CMD]))
            # print(sess.run(batch_idx_branch_idx))
            print(sess.run(pred_all_branches))
            print("\n")
