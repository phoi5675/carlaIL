...
flags = tf.app.flags
FLAGS = flags.FLAGS
tf.flags.DEFINE_string("preproc_config_paths", default="/imitation/config-preprocess-production.yaml",
...
def read_h5_file(h5_file):
    ...
    try:
        ...
        for batch_idx in range(batch_size):
            ...
            # undersampling
            # these constants are bit biased
            hlc = targets[24]
            steer = targets[0]
            rand = random.uniform(0.0, 1.0)

            # a small percentage of the data has no intention; remove these examples
            # lanefollow : undersampling (-0.1, 0.1) part
            # left / right : steer > 0.5 or -0.5
            # or (hlc == 4.0 and abs(steer) < 0.001 and 0 <= rand <= 0.30) \
            #                     or (hlc == 1.0 and steer > 0.8) or (hlc == 2.0 and steer < -0.8)
            #  or (hlc == 4.0 and rand <= 0.5)
            intention_idx = ilc.TGT_KEYS.index(ilc.TGT_HIGH_LVL_CMD)
            intention = targets[intention_idx]
            if intention is None or (hlc == 5.0 or hlc == 6.0) or (hlc == 4.0 and rand <= 0.6):
        ...
        logging.error("Failure for file %s . error : %s Skipping.", h5_file, e)
...
def run_pipeline():
    for label, subdir in [(VAL_SPLIT, 'SeqVal'), (TRAIN_SPLIT, 'SeqTrain')]:
        ...
        for index in range(0, len(h5_files), BATCHSIZE):
            ...
            output_filename = '{}-{:05f}.tfrecord.gz'.format(label, index / BATCHSIZE)
