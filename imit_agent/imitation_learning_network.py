...
class Network(object):
    ...
    def conv(self, x, kernel_size, stride, output_size, padding_in='SAME'):
        ...
        filters_in = x.get_shape()[-1]  # x = input_image 또는 input_image 를 이용한 conv. network 의 결과물
    ...
    def conv_block(self, x, kernel_size, stride, output_size, padding_in='SAME'):
        ...
        with tf.name_scope("conv_block" + str(self._count_conv)):
            # self.conv -> convolution layer 생성. 이 함수 내부까지 볼 필요는 없을수도?
            x = self.conv(x, kernel_size, stride, output_size, padding_in=padding_in)
            # batch_normalization. 학습 성능을 높여주는 방법
            x = self.bn(x)
            # overfit 을 줄이기 위해 뉴런과 연결 일부를 제거하는 과정
            x = self.dropout(x)

            # sigmoid 함수 대신 신경망 학습이 잘 이루어지게 (x < 0, y = 0 / x >= 0 y = x) 와 유사한 함수로 대체
